...
class ExperimentTracker:
    """Small optional W&B adapter shared by RotGS trainers."""

    RUN_ID_FILENAME = "wandb_run_id"
    PORTABLE_PATH_FIELDS = ("start_checkpoint", "init_geometry", "init_motion")
    ...
    def create(cls, args, output_path):
        ...
        # instead of machine-specific dataset and initializer locations.
        config.pop("source_path", None)
        for field in cls.PORTABLE_PATH_FIELDS:
            if config.get(field):
                config[field] = os.path.basename(os.path.normpath(config[field]))
        ...
        try:
            ...
            config["git_branch"] = subprocess.check_output(
        ...
        is_multi_camera = bool(getattr(args, "multi_camera", False))
        run = wandb.init(
            ...
            job_type="multi_camera_train" if is_multi_camera else "train",
            save_code=False,
